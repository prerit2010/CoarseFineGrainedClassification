...
fine_classes = {"aircrafts" : 7, "dogs" : 5, "birds" : 11, "cars": 8 , "flowers" : 5}
...
class BCNN(torch.nn.Module):
    ...
    def __init__(self):
        ...
        self.features = torch.nn.Sequential(*list(self.features.children())
                                            [:-1])  # Remove pool5.
        # Linear classifier.
        self.fc = torch.nn.Linear(512**2, fine_classes[coarse_class])
    ...
    def forward(self, X):
        ...
        X = self.fc(X)
        assert X.size() == (N, fine_classes[coarse_class])
        return X
...
#         print('Best at epoch %d, test accuaray %f' % (best_epoch, best_acc))
        self._accuracy(self._test_loader)
#         print(test_acc, "dsds")
...
#             print(x)
            x = [str(i) for i in x]
#             print(x)
            y = [i.split('(')[1].split(',')[0] for i in x]
#             print(y)
            cc = coarse_class
            base_path = "data/testing/" + cc
            image_names = pickle.load(open(base_path + '/test_file_names.pkl', 'rb'))
#             print(len(image_names), len(y))
            f_ans = open(base_path + "/final_labels.txt", "w")
            for j, i in enumerate(image_names):
                var1 = i + " " + cc + " " + cc + "@" + str(int(y[j])+1)
                f_ans.write("%s\n" % var1)
            f_ans.close()
#             num_total += y.size(0)
...
def main():
    ...
    parser.add_argument('--clas', dest='clas', type=str, required=True,
                        help='Set a coarse grained class')
