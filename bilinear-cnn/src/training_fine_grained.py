...
#         self.features = torchvision.models.resnet18(pretrained=True)
        self.features = torch.nn.Sequential(*list(self.features.children())
                                            [:-1])  # Remove pool5.
        # Linear classifier.
        self.fc = torch.nn.Linear(512**2, fine_classes[cub200.coarse_class])
#         self.fc = torch.nn.Linear(512, 5) #change
#         pytorch_total_params = sum(p.numel() for p in model.parameters() if p.requires_grad)
    ...
    def forward(self, X):
        ...
        X = self.fc(X)
        assert X.size() == (N, fine_classes[cub200.coarse_class])
        return X
...
#             transform=test_transforms)
        ...
        train_data = cub200.CUB200(
            root=self._path[cub200.coarse_class], train=True, download=True,
            transform=train_transforms)
        test_data = cub200.CUB200(
            root=self._path[cub200.coarse_class], train=False, download=True,
            transform=test_transforms) #change
    ...
    def train(self):
        ...
        for t in range(self._options['epochs']):
            ...
            for X, y in self._train_loader:
                ...
                # Forward pass.
                score = self._net(X)
#                 score = self.features.forward(X) #change
            ...
            if test_acc >= best_acc:
                ...
                model_name = cub200.coarse_class + '_vgg_16_epoch_{}.pth'.format(t+1)
...
#             transform=torchvision.transforms.ToTensor(), download=True)
        ...
        train_data = cub200.CUB200(
            root=self._path[cub200.coarse_class], train=True,
            transform=torchvision.transforms.ToTensor(), download=True) #change
