...
clt = DBSCAN(eps=3, min_samples=2)

labels1 = clt.fit_predict(image1)
# quant1 = clt.cluster_centers_#.astype("uint8")[labels1]
quant1 = clt.components_.astype("uint8")[labels1]
print("1st done")
print("components_", quant1.shape)
print("core_sample_indices_",clt.core_sample_indices_.shape)
print("labels_",clt.labels_.shape)
exit()
...
print(quant1.shape,labels1.shape)
...
# os.mkdir(path)
cv2.imwrite(os.path.join(path , 'image1reducedsasa.jpg'), quant1)
