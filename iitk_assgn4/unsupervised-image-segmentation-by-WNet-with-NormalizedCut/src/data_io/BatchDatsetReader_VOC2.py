...
def create_BatchDatset(im_size):
...
#     data_record = read_data_record('./data/VOCdevkit/VOC2012')
    images = os.listdir("../../Dataset/camera1/JPEGImages")
    train_dataset = BatchDatset(images, False, im_size)
...
class BatchDatset:
    ...
    def __init__(self, data_records, is_shuffle=False, im_size=128):
        """
        
        """
        
        print("Initializing Batch Dataset Reader...")
        self.read_data_to_self(data_records, im_size)
    ...
    def read_data_to_self(self, data_records, resize_size = 128):
        ...
        z = []
        count = 0
        for datum in data_records:
            try:
                a = misc.imresize(imageio.imread("../../Dataset/camera1/JPEGImages/" + datum), 
                        [resize_size, resize_size], interp='bilinear')
                z.append(a)
            except:
                count+=1
        print(len(z), count)
        self.images = np.stack(z)
#         self.images = np.stack([misc.imresize(imageio.imread("imageio:" + "../../Dataset/camera1/JPEGImages/" + datum), 
#                         [resize_size, resize_size], interp='bilinear') for datum in data_records if ".jpg" in datum ])                        
