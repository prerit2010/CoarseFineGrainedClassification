...
from data_io.BatchDatsetReader_VOC2 import create_BatchDatset
...
class Wnet_naive(Unet):
    ...
    def train_net(self, train_dataset_reader):
        ...
        for itr in range(self.flags.max_iteration):
            ...
            if itr % 10 == 0:
                ...
                print("Step: %d, Train_loss:%g" % (itr, train_loss), time.time()-st_time)
                st_time = time.time()
...
#                 self.saver.save(self.sess, os.path.join(self.flags.logs_dir, "model.ckpt"), itr)
    ...
    def visaulize_pred(self, dataset_reader):
        ...
        valid_images = dataset_reader.get_random_batch(self.flags.batch_size)
...
if __name__ == '__main__':
    ...
    if flags.mode == "test":
        ...
        train_dataset_reader = create_BatchDatset(int(flags.image_size))
        
        if flags.mode == "train":
            net.train_net(train_dataset_reader)
            
        elif flags.mode == "visualize":
            valid_images, preds = net.visaulize_pred(train_dataset_reader)
